...
(defn isa?
  ...
  ([child parent] (isa? temple/*tree-of-life* child parent))
...
(defn ancestors
  ...
  ([tag] (ancestors temple/*tree-of-life* tag))
...
(defn descendants
  ...
  ([tag] (descendants temple/*tree-of-life* tag))
...
(defn parents
  ...
  must be a hierarchy obtained from `make-hierarchy`, if not supplied
  defaults to `*tree-of-life*`."
  ([tag] (parents temple/*tree-of-life* tag))
...
(defmulti shared-initialize
  ...
  {:arglists '([instance slot-names & initargs])}
...
(defmulti update-instance-for-different-class
  ...
  {:arglists '([previous current & initargs])}
...
(defmulti update-instance-for-redefined-class
  ...
  {:arglists '([instance added-slots discarded-slots property-list & initargs])}
  type-of
  :hierarchy #'net.wikipunk.temple/*tree-of-life*)
